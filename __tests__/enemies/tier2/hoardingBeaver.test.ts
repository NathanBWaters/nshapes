import { createHoardingBeaver } from '@/utils/enemies/tier2/hoardingBeaver';
import type { RoundStats } from '@/types/enemy';
import type { Card } from '@/types';

const createEmptyStats = (): RoundStats => ({
  totalMatches: 0,
  currentStreak: 0,
  maxStreak: 0,
  invalidMatches: 0,
  matchTimes: [],
  timeRemaining: 60000,
  cardsRemaining: 12,
  tripleCardsCleared: 0,
  faceDownCardsMatched: 0,
  bombsDefused: 0,
  countdownCardsMatched: 0,
  shapesMatched: new Set(),
  colorsMatched: new Set(),
  colorMatchCounts: new Map(),
  allDifferentMatches: 0,
  allSameColorMatches: 0,
  squiggleMatches: 0,
  gracesUsed: 0,
  hintsUsed: 0,
  hintsRemaining: 3,
  gracesRemaining: 2,
  damageReceived: 0,
  weaponEffectsTriggered: new Set(),
  currentScore: 0,
  targetScore: 100,
});

const createTestBoard = (): Card[] =>
  Array(12)
    .fill(null)
    .map((_, i) => ({
      id: `card-${i}`,
      shape: 'oval' as const,
      color: 'red' as const,
      number: 1 as const,
      shading: 'solid' as const,
      selected: false,
    }));

describe('Hoarding Beaver', () => {
  describe('metadata', () => {
    it('has correct name and tier', () => {
      const enemy = createHoardingBeaver();
      expect(enemy.name).toBe('Hoarding Beaver');
      expect(enemy.tier).toBe(2);
    });

    it('has correct icon', () => {
      const enemy = createHoardingBeaver();
      expect(enemy.icon).toBe('delapouite/beaver');
    });
  });

  describe('extra card removal on match', () => {
    it('removes 1 extra card on valid match', () => {
      const enemy = createHoardingBeaver();
      const board = createTestBoard();

      const result = enemy.onValidMatch([board[0], board[1], board[2]], board);
      expect(result.cardsToRemove.length).toBe(1);
    });

    it('does not remove if at minimum board size', () => {
      const enemy = createHoardingBeaver();
      const board = createTestBoard().slice(0, 6);

      const result = enemy.onValidMatch([board[0], board[1], board[2]], board);
      expect(result.cardsToRemove.length).toBe(0);
    });
  });

  describe('card removal effect', () => {
    it('removes card after 18 seconds', () => {
      const enemy = createHoardingBeaver();
      const board = createTestBoard();
      enemy.onRoundStart(board);

      const result = enemy.onTick(18000, board);
      expect(result.cardsToRemove.length).toBe(1);
    });
  });

  describe('defeat condition', () => {
    it('returns false if target not reached', () => {
      const enemy = createHoardingBeaver();
      const stats = createEmptyStats();
      stats.currentScore = 80;
      stats.targetScore = 100;
      stats.cardsRemaining = 8;
      expect(enemy.checkDefeatCondition(stats)).toBe(false);
    });

    it('returns false if fewer than 6 cards remaining', () => {
      const enemy = createHoardingBeaver();
      const stats = createEmptyStats();
      stats.currentScore = 100;
      stats.targetScore = 100;
      stats.cardsRemaining = 5;
      expect(enemy.checkDefeatCondition(stats)).toBe(false);
    });

    it('returns true if target reached with 6+ cards', () => {
      const enemy = createHoardingBeaver();
      const stats = createEmptyStats();
      stats.currentScore = 100;
      stats.targetScore = 100;
      stats.cardsRemaining = 6;
      expect(enemy.checkDefeatCondition(stats)).toBe(true);
    });
  });
});
